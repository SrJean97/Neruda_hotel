...
export class ReservaConsultaComponent implements OnInit {
  ...
  tomarValor(texto:string){
    ...
    if(texto==''){
      const messageBox = this.modalService.open(AlertModalComponent)
      messageBox.componentInstance.title = "Resultado Operación";
      messageBox.componentInstance.message = 'Identificación invalida'
    }else{
  ...
  fechaCorrecta(){ 
    ...
    if(toma1>toma2){
      ...
      const messageBox = this.modalService.open(AlertModalComponent)
      messageBox.componentInstance.title = "¡Problemas!";
      messageBox.componentInstance.message = 'No se puede reservar en fechas anterioreas a la actual.';      
    }else{
